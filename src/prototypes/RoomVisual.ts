...
RoomVisual.prototype.resource = function(type, x, y, size = 0.25){
    const minerals: string[] = [RESOURCE_CATALYST, RESOURCE_HYDROGEN, RESOURCE_OXYGEN, RESOURCE_LEMERGIUM, RESOURCE_UTRIUM, RESOURCE_ZYNTHIUM, RESOURCE_KEANIUM];
    if (type == RESOURCE_ENERGY || type == RESOURCE_POWER)
        this.fluid(type, x, y, size);
    else if (minerals.indexOf(type))
